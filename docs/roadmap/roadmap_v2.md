...
### 2.2 プラン案内ページの実装 (`/pricing`) (完了)
- [x] **2.2.1 ページレイアウト作成**: Cron風のミニマルデザイン。
- [x] **2.2.2 プラン比較表の実装**: Free, Premium, Early Adopter の機能差を可視化。
- [x] **2.2.3 リアルタイム枠数表示**: `/api/stats/count` を再利用し、残り枠数を表示。
...
### 2.4 リーガルページの作成 (完了)
- [x] **2.4.1 利用規約 (`/terms`)**: シンプルで読みやすい規約ページ。
- [x] **2.4.2 プライバシーポリシー (`/privacy`)**: データ保護方針の明記。
