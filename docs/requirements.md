...
- **プラン別機能一覧**:
  - **未ログイン (Guest)**:
    ...
    - ローカルでの通知
  - **ログイン・無料プラン (Free)**:
    - 設定のローカル永続化（localStorage）
    - 基本的な統計（今日の実績）の閲覧
    - ログインユーザー限定の洗練されたUI
  - **ログイン・アーリーアダプター (👑 Early Adopter - 先着15名)**:
    - **全プレミアム機能の永久無償解放**
    - 設定のクラウド同期（複数デバイス間）
    - 高度な統計・分析ダッシュボード
    - 外部連携機能（将来的に追加されるすべてのツール）
    - 👑 限定バッジの付与
  - **ログイン・プレミアムプラン (Premium - 将来提供)**:
    - 設定のクラウド同期
    - 高度な分析機能
    - カスタムアラーム音の設定
    - 外部アプリ連携（Notionなど）
...
- **早期アダプター特典の訴求**:
  - ページ内に「残り枠数」をリアルタイム表示し、希少性を高める。
  - 15名に達した後は自動的に通常のプラン案内に切り替える。
