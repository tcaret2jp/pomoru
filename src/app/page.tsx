...
import { FeatureGateModal } from '@/components/features/auth/FeatureGateModal';
...
export default function Home() {
  ...
  // Feature Gate state
  const [gateType, setGateType] = useState<'login' | 'plus' | 'premium' | null>(null);
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (distance > 50) { setIsNavOpen(false); setTouchStart(null); }
  };
  const handleTouchEnd = () => setTouchStart(null);

  const [playAlarm] = useSound('/alarm.mp3', { volume: 0.5 });

  const { timeLeft, isActive, mode, start, pause, reset, switchMode } = useTimer(settings);
  ...
  useEffect(() => {
    if (timeLeft <= 0 && !hasTriggeredComplete.current && isActive) {
      ...
      if (mode === 'work') {
        settings.autoStartBreaks ? switchMode('shortBreak') : setIsFlowModeOpen(true);
      } else {
        switchMode('work');
        if (!settings.autoStartWork) pause();
      }
    }
    if (timeLeft > 0) hasTriggeredComplete.current = false;
  ...
  useEffect(() => {
    ...
    const timeString = `${Math.floor(absTime / 60).toString().padStart(2, '0')}:${(absTime % 60).toString().padStart(2, '0')}`;
    document.title = `${timeLeft < 0 ? '+' : ''}${timeString} - Pomoru`;
  }, [timeLeft]);
  ...
  return (
    <main className="flex min-h-[100dvh] flex-col items-center bg-background text-foreground transition-colors duration-300 px-4 relative overflow-hidden text-center">
          ...
          <div className="flex items-center gap-2 w-full max-w-[280px] mx-auto relative" ref={taskMenuRef}>
            <div className="flex-1 flex items-center bg-muted/30 border border-border/50 rounded-2xl overflow-hidden h-10 group/task shadow-sm">
              ...
              >
                ...
                <span className={cn("text-[10px] font-bold uppercase tracking-wider truncate", selectedTask ? "text-foreground" : "text-muted-foreground group-hover/task:text-foreground")}>
            ...
            {isTaskMenuOpen && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-card border border-border rounded-2xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="p-2">
                  ...
                  {todayTasks.map((task) => (
                    <button key={task} onClick={() => { setSelectedTask(task); setIsTaskMenuOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-muted transition-colors text-left group">
                      <div className={cn("w-4 h-4 rounded-full border flex items-center justify-center shrink-0 transition-colors", selectedTask === task ? "bg-primary border-primary" : "border-muted-foreground/30 group-hover:border-primary/50")}>
                        {selectedTask === task && <CheckCircle2 className="w-2.5 h-2.5 text-white" />}
                      </div>
                      <span className={cn("text-xs font-medium truncate", selectedTask === task ? "text-foreground" : "text-muted-foreground group-hover:text-foreground")}>{task}</span>
                    </button>
                  ))}
                  <div className="px-3 py-2 mb-1 bg-primary/5 rounded-xl border border-primary/10">
                    <p className="text-[8px] font-bold text-primary leading-tight">※ 現在はプレビュー版です。実際のタスク管理機能は近日公開予定。</p>
                  </div>
                  <div className="h-[1px] bg-border my-2" />
                  <button onClick={handleViewAllTasks} className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl hover:bg-primary/5 transition-colors group">
            ...
            <button onClick={handleStatsClick} className="flex items-center justify-center w-10 h-10 rounded-2xl bg-muted/30 border border-border/50 hover:bg-muted/50 transition-all text-lg hover:bg-primary/5 active:scale-95">
              📊
            </button>
          </div>
        </div>
        
        <TimerProgress timeLeft={timeLeft} totalTime={settings[mode]} mode={mode} size={320} className="mx-auto w-[280px] h-[280px] md:w-[320px] md:h-[320px]">
          <TimerDisplay timeLeft={timeLeft} isEditable={!isActive} onEdit={handleTimerClick} />
        </TimerProgress>
        
        <TimerControls isActive={isActive} onToggle={isActive ? pause : start} onReset={reset} onOpenSettings={() => setIsSettingsOpen(true)} variant={mode === 'work' ? 'primary' : 'secondary'} />
      </div>

      <SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} settings={settings} onSave={saveSettings} />
      <FlowModeDialog isOpen={isFlowModeOpen} timeLeft={timeLeft} onTakeBreak={handleTakeBreak} onFinish={handleFinish} />
      <WelcomeModal isOpen={isWelcomeOpen} onClose={() => setIsWelcomeOpen(false)} />
      <TimeEditModal isOpen={isTimeEditOpen} onClose={() => setIsTimeEditOpen(false)} mode={mode} initialSeconds={settings[mode]} onSave={(newSeconds) => saveSettings({ ...settings, [mode]: newSeconds })} />
      
      {/* Feature Gate Modal */}
      <FeatureGateModal 
        isOpen={gateType !== null} 
        onClose={() => setGateType(null)} 
        type={gateType || 'login'} 
      />

      <div className="absolute bottom-8 left-0 right-0 flex flex-col items-center gap-4 text-center">
        <button onClick={() => setIsNavOpen(true)} className="group flex flex-col items-center gap-1.5 px-6 py-2 rounded-full hover:bg-muted/50 transition-all active:scale-95 mx-auto">
      ...
      <div className={cn("fixed inset-0 bg-background/40 backdrop-blur-md z-40 transition-all duration-500", isNavOpen ? "opacity-100 visible" : "opacity-0 pointer-events-none invisible")} onClick={() => setIsNavOpen(false)} />
      <div className={cn("fixed bottom-0 left-0 right-0 bg-card border-t border-border z-50 transition-transform duration-500 ease-out rounded-t-[2.5rem] shadow-2xl pb-12 pt-4 px-6", isNavOpen ? "translate-y-0" : "translate-y-full")}>
        <div className="max-w-md mx-auto text-center">
          <div className="w-12 h-1.5 bg-muted rounded-full mx-auto mb-8 cursor-grab active:cursor-grabbing touch-none" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} onClick={() => setIsNavOpen(false)} />
          <div className="grid grid-cols-1 gap-3">
            <Link href="/pricing" onClick={() => setIsNavOpen(false)} className="flex items-center gap-4 p-5 rounded-3xl bg-muted/30 hover:bg-muted transition-colors group text-left">
              <div className="p-3 rounded-2xl bg-background shadow-sm group-hover:scale-110 transition-transform"><CreditCard className="w-5 h-5 text-primary" /></div>
            ...
            <Link href="/about" onClick={() => setIsNavOpen(false)} className="flex items-center gap-4 p-5 rounded-3xl bg-muted/30 hover:bg-muted transition-colors group text-left">
              <div className="p-3 rounded-2xl bg-background shadow-sm group-hover:scale-110 transition-transform"><Info className="w-5 h-5 text-primary" /></div>
              <div className="flex-1 min-w-0">
            ...
            {!isEarlyAdopter && (
              <Link href="/early-access" onClick={() => setIsNavOpen(false)} className="flex items-center gap-4 p-5 rounded-3xl bg-primary/5 border border-primary/10 hover:bg-primary/10 transition-colors group text-left">
                <div className="p-3 rounded-2xl bg-primary/10 shadow-sm group-hover:scale-110 transition-transform"><Sparkles className="w-5 h-5 text-primary" /></div>
                <div className="flex-1 min-w-0">
